...
interface QueueEntry {
  keys: Set<number>;
  /** ms epoch of the most recent enqueue */
  lastEnqueueAt: number;
  /** ms epoch when the current debounce timer fires (0 if none) */
  flushAt: number;
  idleTimer?: NodeJS.Timeout;
  /** Set true while a flush is in flight; new enqueues just append. */
  flushing: boolean;
}
 ...
 */
type RunBackfill = (
  opts: import("../handlers/backfill.js").BackfillOptions
) => Promise<unknown>;
let runBackfillRef: RunBackfill | null = null;
export function setRunBackfill(fn: RunBackfill): void {
  runBackfillRef = fn;
}
...
function ensureEntry(projectId: string): QueueEntry {
...
export function enqueueTranslation(
  projectId: string,
  keyId: number
): { queued: boolean; queueSize: number; flushAt: number } {
  const project = getProject(projectId);
  if (!project) return { queued: false, queueSize: 0, flushAt: 0 };
  ...
  if (entry.keys.size >= maxKeys) {
    logger().info(
      { projectId, queueSize: entry.keys.size, maxKeys },
      "Translation queue hit max-keys cap — flushing immediately"
    );
...
export async function flush(
  projectId: string,
  reason: "idle" | "max-keys" | "scheduled" | "manual"
): Promise<{ submitted: boolean; keyIds: number[]; reason: string } | null> {
  const entry = queues.get(projectId);
  if (!entry || entry.keys.size === 0) return null;
  if (entry.flushing) {
    logger().debug(
      { projectId, queueSize: entry.keys.size, reason },
      "Flush requested while another flush is in flight — skipped"
    );
    return null;
  }
  ...
  logger().info(
    { projectId, keyCount: keyIds.length, reason },
    "Flushing translation queue"
  );
  ...
  try {
    if (!runBackfillRef) {
      logger().error(
        { projectId },
        "runBackfill not registered — translation queue cannot flush"
      );
      // Re-add the keys so we don't lose them
      for (const k of keyIds) entry.keys.add(k);
      return null;
    }
...
export function getAllQueueStates(): QueueState[] {
...
export function getQueueState(projectId: string): QueueState | null {
  const entry = queues.get(projectId);
