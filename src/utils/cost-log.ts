...
import { calculateCost, formatUsd, type CostBreakdown } from "./cost.js";
...
export async function recordCost(args: {
  ...
  try {
    ...
    log().warn(
      ...
      "Failed to record cost (non-fatal)"
    );
    return undefined;
  }
}

async function readAllEntries(): Promise<CostEntry[]> {
  ...
  for (const line of content.split("\n")) {
    ...
    if (!trimmed) continue;
    try {
      out.push(JSON.parse(trimmed) as CostEntry);
    } catch {
...
function addInto(agg: CostAggregate, e: CostEntry): void {
  agg.calls++;
...
function decorate(agg: CostAggregate): FormattedAggregate {
  return {
    ...agg,
...
function decorateEntry(e: CostEntry): FormattedCostEntry {
  return {
    ...e,
...
function renderText(
  total: CostAggregate,
  byProjectAndLanguage: Record<string, Record<string, CostAggregate>>,
  byProjectAndModel: Record<string, Record<string, CostAggregate>>,
...
export async function summarizeCosts(
  ...
  projectNameLookup?: (id: string) => string
): Promise<CostSummary> {
  ...
  const byProjectRaw: Record<string, CostAggregate> = {};
  const byProjectAndLanguageRaw: Record<string, Record<string, CostAggregate>> = {};
  const byProjectAndModelRaw: Record<string, Record<string, CostAggregate>> = {};

  for (const e of filtered) {
    addInto(total, e);

    const pKey = e.projectId ?? "_none";
    if (!byProjectRaw[pKey]) byProjectRaw[pKey] = emptyAgg();
    addInto(byProjectRaw[pKey], e);

    if (!byProjectAndLanguageRaw[pKey]) byProjectAndLanguageRaw[pKey] = {};
    ...
    if (!byProjectAndModelRaw[pKey]) byProjectAndModelRaw[pKey] = {};
  ...
  const decorateMap = <T>(
    raw: Record<string, T>,
    fn: (v: T) => FormattedAggregate
  ): Record<string, FormattedAggregate> => {
    const out: Record<string, FormattedAggregate> = {};
    for (const [k, v] of Object.entries(raw)) out[k] = fn(v);
    return out;
  };

  const byProject = decorateMap(byProjectRaw, decorate);
  const byProjectAndLanguage: Record<string, Record<string, FormattedAggregate>> = {};
  for (const [pid, langs] of Object.entries(byProjectAndLanguageRaw)) {
    byProjectAndLanguage[pid] = decorateMap(langs, decorate);
  }
  const byProjectAndModel: Record<string, Record<string, FormattedAggregate>> = {};
  ...
  const lookup = projectNameLookup ?? ((id: string) => id);
  const formatted = renderText(
    total,
    byProjectAndLanguageRaw,
    byProjectAndModelRaw,
    lookup
  );
