...
async function adaptLokaliseEvent(
  ...
  if (!projectId || !keyId || !editedLanguage) return null;
...
function parseWebhookPath(path: string): {
  isWebhook: boolean;
  projectIdFromPath?: string;
} {
  const m = WEBHOOK_PATH_RE.exec(path);
  if (!m) return { isWebhook: false };
...
app.use((req: Request, res: Response, next) => {
  ...
  const bodyProjectId: string | undefined = req.body?.project?.id
  ...
  if (projectIdFromPath && bodyProjectId && projectIdFromPath !== bodyProjectId) {
    logger.warn(
      { path: req.path, projectIdFromPath, bodyProjectId },
      "Webhook URL projectId does not match payload project.id"
    );
    return res
      .status(400)
...
app.get("/status", (req: Request, res: Response) => {
  ...
  const errorsLastHour = events.filter(
    (e) => e.type === "error" && Date.now() - e.timestamp < 3600_000
  ).length;
...
app.get("/cost", async (req: Request, res: Response) => {
  try {
    ...
    const lookup = (id: string) =>
      projectsList.find((p) => p.id === id)?.name ?? id;
...
app.post("/trigger/backfill", (req: Request, res: Response) => {
  const received =
    (req.headers["x-secret"] as string) ||
    (req.headers["x-api-key"] as string);
  ...
  runBackfill({
    ...
    requireReviewedSource:
      typeof body.requireReviewedSource === "boolean"
 ...
 */
app.get("/admin/logs/recent", (req: Request, res: Response) => {
  if (!checkAdminSecret(req, res)) return;
 ...
 */
app.get("/admin/logs/stream", (req: Request, res: Response) => {
  if (!checkAdminSecret(req, res)) return;
...
app.get(
  ["/webhook", "/webhooks", "/webhook/:projectId", "/webhooks/:projectId"],
...
app.post(
  ...
  const xEvent = (req.headers["x-event"] as string) || "";
  ...
  try {
    ...
    const incomingProjectId: string | undefined =
    ...
    try {
      ...
      logger.error(
        ...
        "Failed to load project base language"
      );
    ...
    if (events === null) {
      logger.warn(
        { eventId, xEvent, body: req.body },
        "Unrecognised or incomplete webhook event"
      );
