...
export function startScheduler() {
    ...
    logger.info({ cron: expression, timezone: Intl.DateTimeFormat().resolvedOptions().timeZone }, "Scheduling periodic backfill");
    const task = cron.schedule(expression, async () => {
        ...
        try {
            ...
            logger.error({ error: err instanceof Error ? err.message : String(err) }, "Scheduled backfill threw");
        }
    });
    return task;
}
