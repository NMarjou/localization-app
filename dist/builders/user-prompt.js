...
export class UserPromptBuilder {
    ...
    buildUserPrompt(request) {
        this.logger.debug({ language: request.target_language, strings: request.strings.length }, "Building user prompt");
        const sections = [];
        ...
        this.logger.debug({ length: fullPrompt.length }, "User prompt built");
        return fullPrompt;
    }
    formatLanguageInfo(request) {
        const locale = request.target_locale
            ? ` (${request.target_locale})`
            : "";
        return `Target Language: ${request.target_language}${locale}`;
    }
    formatContextSection(title, strings) {
        const items = strings
