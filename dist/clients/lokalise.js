...
export class LokaliseClient {
    ...
    async bulkUpdateTranslations(updates) {
        ...
        for (let i = 0; i < updates.length; i += CONCURRENCY) {
            ...
            const results = await Promise.allSettled(batch.map(({ translationId, translation }) => this.http.put(`/projects/${this.projectId}/translations/${translationId}`, { translation, is_reviewed: false, is_unverified: true })));
        ...
        if (failed > 0) {
            this.logger.warn({ pushed, failed, total: updates.length }, "Bulk translation push completed with some failures");
        }
        this.glossaryCache.clear();
 ...
 */
export function clearAllLokaliseClients() {
